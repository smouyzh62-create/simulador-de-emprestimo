...
export default function Admin() {
  ...
  const addWs = () => {
    ...
    setConfig(prev => ({
      ...prev,
      whatsappPool: [...prev.whatsappPool, newWs.trim()],
    }));
    setNewWs('');
  };
  ...
  const removeWs = (index: number) => {
    setConfig(prev => ({
      ...prev,
      whatsappPool: prev.whatsappPool.filter((_, i) => i !== index),
    }));
  };

  const updateWs = (index: number, value: string) => {
    setConfig(prev => {
      const pool = [...prev.whatsappPool];
      pool[index] = value;
      return { ...prev, whatsappPool: pool };
    });
  };
  ...
  const addTg = () => {
    ...
    setConfig(prev => ({
      ...prev,
      telegramPool: [...prev.telegramPool, newTg.trim()],
    }));
    setNewTg('');
  };
  ...
  const removeTg = (index: number) => {
    setConfig(prev => ({
      ...prev,
      telegramPool: prev.telegramPool.filter((_, i) => i !== index),
    }));
  };

  const updateTg = (index: number, value: string) => {
    setConfig(prev => {
      const pool = [...prev.telegramPool];
      pool[index] = value;
      return { ...prev, telegramPool: pool };
    });
  };
